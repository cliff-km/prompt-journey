    ...
    async function fetchModels(openaiKey) {
        ...
        if (response.data)
            models = response.data.data
                .map(({ id }) => id)
                .filter((id) => supportedModels.includes(id));
    }
...
>
    <div class="flex justify-center">
        <div class="form-control w-full max-w-xs">
            ...
            <input
                value={$key}
                on:input={(e) => keyStore.updateKey(e.target.value)}
                type="password"
    ...
    {#if models.length > 0}
        <div class="flex justify-center">
            <div class="form-control w-full max-w-xs mb-4">
                ...
                >
                    <option disabled selected={selectedModel === null}
                        >Pick one</option
                    >
                    {#each models as m (m)}
                        <option value={m} selected={selectedModel === m}
                            >{m}</option
                        >
                    {/each}
                </select>
