<script lang="ts">
    ...
    $: maxBarWidth = actualWidth - textBarPad - textWidth - 2 * horizontalPad - handleWidth;
    $: barX = xy[0] + textBarPad + textWidth + horizontalPad;
    $: maxBarWeight = Math.max(
        ...Object.values($activePrompt.weightedPrompts).map((p) => p.barWeight || 0)
    );
    ...
    function handleMouseMove(e: Event) {
        if (activeDragId) {
            ...
            let mouseLocation = [loc.x, loc.y];

            let barLength = Math.min(maxBarWidth, Math.max(0, mouseLocation[0] - barX));
            let barRatio = barLength / maxBarWidth;
            
            let prompts = {...$activePrompt.weightedPrompts }
            let wp = { ...prompts[activeDragId] };
...
on:mouseup={handleMouseUp}>
    {#each prompts as [id, idx, p] (id)}
        <PromptText
            ...
            color={`rgba(255,255,255,${getWeightOpacity(p.barWeight/maxBarWeight || 0)})`}
