<script lang="ts">
    ...
    import { preferredDirectiveStore, preferredDirective } from '../lib/prefferedDirectiveStore.js';
    import { metaPromptStore, metaPrompt } from '../lib/metaPromptStore.js';
    import { createOpenAI, createCompletion, createChatCompletion } from '../lib/openai.js';
    ...
    $: directives = Object.entries({...builtInDirectives, ...userDirectives}).reduce((acc, [id, data]) => {
        acc[id] = data;
        return acc;
    }, {});
    ...
    function saveToStore() {
        ...
        directiveStore.updateDirective(id, {name: newPromptName, text: directiveText, builtIn: false});
    ...
    $: isUserDirective = selectedDirective && selectedDirective !== 'new' && !directives[selectedDirective].builtIn;
</script>
    ...
    {#if openaiKey}
        <div class="flex justify-center">
            <div class="form-control w-full max-w-xs mb-4">
                ...
                <select value={selectedDirective} on:change={handleSelectedDirectiveChange} class="select select-bordered">
                    <option disabled selected={selectedDirective===null}>Pick one</option>
                    {#each Object.entries(directives) as [id, data] (id)}
                        <option value={id} selected={selectedDirective===id}>{data.name}{data.builtIn ? " (Built-In)" : " (User)"}</option>
                    {/each}
        ...
        {#if selectedDirective}
            {#if !builtInDirective}
                <div class="w-full p-2 h-12 flex justify-between">
                    <input bind:value={newPromptName} type="text" placeholder="Name" class="input input-bordered input-sm w-full max-w-xs" />
                    <button on:click={saveToStore} class="btn btn-sm btn-primary ml-4">Save</button>
                </div>
            {/if}
            <div class="w-full p-2 h-1/2">
                <textarea bind:value={directiveText} class="textarea textarea-bordered w-full h-full" placeholder="GPT Directives"></textarea>
            </div>
            <div class="w-full p-2 h-1/5">
                <textarea value={$metaPrompt} on:input={(e)=>metaPromptStore.updateMetaPrompt(e.target.value)} class="textarea textarea-bordered w-full h-full" placeholder="Describe the prompt to generate."></textarea>
            </div>
            {#if isUserDirective}
                <div class="w-full p-2 h-12 flex justify-between">
                    <button on:click={deleteFromStore} class="btn btn-sm btn-error ml-4">Delete</button>
                    <button on:click={handleGeneratePrompt} class="btn btn-sm btn-primary ml-4">Generate</button>
                </div>
            {:else}
