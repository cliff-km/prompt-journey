<script lang="ts">
    ...
    import BarPrompt from "../components/BarPrompt.svelte";
    ...
    function handleWeightChange(weightsById) {
        ...
        if (
            Object.entries(weights).length ===
            Object.entries(currentPromptData).length
        ) {
    ...
    function saveToStore() {
        promptStore.updatePrompt(uuidv4(), {
            ...controllerData,
            date: Date.now(),
            exponentialScaling: controllerScalingExponential,
            mode: controllerMode,
        });
    ...
    const handleSinglePromptChange = debounce((id, text) => {
        ...
        value = Object.entries(currentPromptData)
            .reduce((acc, [id, point]) => {
                acc.push(`${point.text}::${point.parsedWeight}`);
                return acc;
            }, [] as string[])
    ...
    $: controllerCenter = [
        Math.round(controllerW / 2),
        Math.round(controllerH / 2),
    ];
    $: controllerRadius =
        Math.round(Math.min(controllerW, controllerH) / 2) - 150;
</script>
            ...
            <div class="px-2 py-4 pb-0 flex">
                <div class="w-52 px-2">
                    ...
                    <input
                        type="checkbox"
                        class="toggle"
                        bind:checked={controllerScalingExponential}
                    />
                </div>
